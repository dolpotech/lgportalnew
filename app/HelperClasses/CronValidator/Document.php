...
class Document
{
    ...
    {
        if (Storage::disk('api')->exists('cronjob/documents.json')) {
            DB::table('lg_documents')->truncate();
            $documents = json_decode(file_get_contents(storage_path('app/cronjob' . DIRECTORY_SEPARATOR . 'documents.json')), true);
            $new_documents = [];
