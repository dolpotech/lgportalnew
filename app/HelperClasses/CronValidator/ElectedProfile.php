...
class ElectedProfile
{
    ...
    {
        if (Storage::disk('api')->exists('cronjob/elected-profile.json')) {
            DB::table('elected_profiles')->truncate();
            $elected_profiles = json_decode(file_get_contents(storage_path('app/cronjob' . DIRECTORY_SEPARATOR . 'elected-profile.json')), true);
