...
<!--        </div>-->
        <div class="card-footer">
            <button type="button" @click="updateUsers" class="btn btn-info">Update User</button>
...
export default {
    ...
    methods: {
        updateUsers() {
            ...
            // }
            axios.post(this.config.getAdminUserUrl+"/updateUser", {
                id: this.user.id,
                role: this.role,
                type: this.type,
        ...
        setValue(){
            ...
            if(this.user.lg_id)
            {
                this.lg_role = this.user.local_government.id;
            }
            if(this.user.ministry_id)
            {
                this.ministry_role = this.user.ministry.id;
            }
            if(this.user.office_id)
            {
                this.ministry_office_role = this.user.ministry_office.id;
            }
            if(this.user.ministry_department_id)
            {
                this.ministry_department_role = this.user.ministry_department.id;
            }
        ...
        emptyTypeRoles(){
            if (this.role == 'super_admin' || this.role == 'lg_admin'
                || this.role == 'lg_cao' || this.role == 'lg_officer'){
                this.type = 'local_government'
            }else if(this.role == 'super_admin' || this.role == 'ministry_admin' ||
                this.role == 'ministry_cao' || this.role == 'ministry_officer'){
                this.type = 'ministry'
            }else if(this.role == 'super_admin' || this.role == 'mo_admin' ||
                this.role == 'mo_cao' || this.role == 'mo_officers'){
                this.type = 'ministry_office'
            }
            this.lg_role = '';
            this.ministry_role = '';
            this.ministry_office_role = '';
            this.ministry_department_role = '';
        }
    },
    mounted() {
        this.getRoles();
